...
public class RolloutSimulation {
	...
	private HashMap<String, PreFlopPropability> propMap = new HashMap<String, PreFlopPropability>();
	...
	public RolloutSimulation() {
		...
		if (file.exists())
			...
			System.out
					.println("**The table with preflop propabilities could not be found.\n"
	...
	public void ReadTable(File f) throws IOException {
		try {
			...
			while ((strLine = br.readLine()) != null) {
				...
				PFProps.add(p);
				propMap.put(p.getIdentString(), p);
	...
	public void CalculatePreFlopPropabilities(int R) {
		...
		for (int i = 2; i < 15; i++) {
			for (int j = 2; j <= i; j++) {
				...
				// Prototype hands for all unsuited equivalence classes
				// including pairs
				p = new PreFlopPropability(new Card(Suite.CLUBS, j), new Card(
						Suite.DIAMONDS, i), numPlayers);
				PFProps.add(p);
				propMap.put(p.getIdentString(), p);
			}
		}
		...
		for (int i = 2; i < 15; i++) {
			for (int j = 2; j <= i; j++) {
				// Suited prototypes
				if (i != j) {
					p = new PreFlopPropability(new Card(Suite.CLUBS, j),
							new Card(Suite.CLUBS, i), numPlayers);
					PFProps.add(p);
					propMap.put(p.getIdentString(), p);
				}
			}
	...
	public double PropabilityForRRollouts(ArrayList<Card> Hand, int R,
			int numPlayers) {
	...
	public double getPropabilityFromList(ArrayList<Card> Hand, int numPlayers) {
		...
		String handIdent = normHand.get(0).toString()
				+ normHand.get(1).toString();
		PreFlopPropability p = propMap.get(handIdent);
		if (p != null) {
	...
	public int rollout(ArrayList<Card> PlayerCards,
			ArrayList<Card> CommonCards, int NumPlayers, Deck _d) {
		Deck d = new Deck(_d, true);
		if (CommonCards == null)
			CommonCards = new ArrayList<Card>();
		while (CommonCards.size() < 5)
			CommonCards.add(d.getCard());
		return evalRollout(PlayerCards, CommonCards, NumPlayers, d);
	}

	private int evalRollout(ArrayList<Card> PlayerCards,
			ArrayList<Card> CommonCards, int NumPlayers, Deck d) {
